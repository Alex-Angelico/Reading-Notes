...
<head>
  <meta charset="utf-8">
  <title>{strwb block title strwb}Django Auth Tutorial{strwb endblock strwb}</title>
</head>
<body>
  <main>
    {strwb block content strwb}
    {strwb endblock strwb}
  </main>
</body>
</html>
```
...
```
<!-- templates/home.html -->
{strwb extends 'base.html' strwb}

{strwb block title strwb}Home{strwb endblock strwb}

{strwb block content strwb}
{strwb if user.is_authenticated strwb}
  Hi {{ user.username }}!
  <p><a href="{strwb url 'logout' strwb}">Log Out</a></p>
{strwb else strwb}
  <p>You are not logged in</p>
  <a href="{strwb url 'login' strwb}">Log In</a> |
  <a href="{strwb url 'signup' strwb}">Sign Up</a>
{strwb endif strwb}
{strwb endblock strwb}
```
...
{strwb block title strwb}Log In{strwb endblock strwb}

{strwb block content strwb}
<h2>Log In</h2>
<form method="post">
  {strwb csrf_token strwb}
  {{ form.as_p }}
...
{strwb block title strwb}Sign Up{strwb endblock strwb}
...
To create a DjangoX project, click the "Use This Template" button in the DjangoX GitHub repository, which will initialize a new repository with the user's preferred name (it is not a fork of DjangoX itself). Following this, clone the new repository to a local directory, and refer to the Pipfile to install dependencies via poetry. Afterwards, the following files can be deleted:
...
These files are all unnecessary, as their functionality is now covered by the new toml and poetry lock files.